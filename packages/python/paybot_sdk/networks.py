...
#   - ``'eip3009'`` — gasless ``TransferWithAuthorization`` (USDC/EURC/PYUSD).
#   - ``'eip2612'`` — ERC-2612 ``permit`` (RLUSD/DAI). Not yet implemented by
#     this SDK; signing such a token raises
#     :class:`PayBotUnsupportedSigningMethodError` (documented rejection).
...
#
# USDC/EURC addresses are kept in sync with NETWORKS[...].usdc_address for USDC.
# PYUSD uses eip3009 (PayPal USD supports TransferWithAuthorization). RLUSD and
# DAI declare eip2612 (permit) — present for parity/discovery, but signing them
# is a DOCUMENTED REJECTION until EIP-2612 support lands (see
# PayBotUnsupportedSigningMethodError). None carry mainnet addresses for
# regulated tokens in the public registry.
TOKENS: Dict[str, TokenConfig] = {
    ...
    "PYUSD": TokenConfig(
        ...
        eip712_version="1",
        # No public mainnet address shipped — operator-injected at runtime.
        address_by_network={},
    ),
    "RLUSD": TokenConfig(
        symbol="RLUSD",
        decimals=6,
        name="Ripple USD",
        # eip2612 permit — NOT implemented by this SDK; signing is a documented
        # rejection (PayBotUnsupportedSigningMethodError).
    ...
    "DAI": TokenConfig(
        ...
        decimals=18,
        name="Dai Stablecoin",
        # eip2612 permit — NOT implemented by this SDK; documented rejection.
        signing_method="eip2612",
