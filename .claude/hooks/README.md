...
```
PreToolUse Hooks
├── WebSearch     → inject-current-date.py (MODIFIES query)
│                 → enhance-search-query.py (MODIFIES query)
...
### 1. inject-current-date.py
**Trigger:** `WebSearch`
**Behavior:** MODIFIES tool input (adds current year)

Intercepts WebSearch queries and injects the current year if the query
doesn't already contain temporal context (year, "latest", "current", etc.).

**Examples:**
- `"React hooks tutorial"` → `"React hooks tutorial 2026"`
- `"Claude Code teams 2026"` → unchanged (already has year)
- `"What is JavaScript"` → unchanged (timeless/definitional query)

### 2. enhance-search-query.py
**Trigger:** `WebSearch`
**Behavior:** MODIFIES tool input (adds specificity markers)

Enhances vague technical queries with better search terms:
- Library queries → adds "documentation"
- How-to queries → adds "example code"
- Error queries → adds "solution fix"
- API queries → adds "API reference"

**Examples:**
- `"react hooks"` → `"react hooks documentation"`
- `"how to use prisma"` → `"how to use prisma example code"`
- `"TypeError cannot read property"` → `"TypeError cannot read property solution fix"`

### 3. read-protection.py
**Trigger:** `Read`
...
### 4. enforce-architecture-first.py
**Trigger:** `Write|Edit`
**Comportamento:** BLOQUEIA (exit 2)
